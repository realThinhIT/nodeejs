...
controller.login = (req, res, middleware) => {
    ...
    authenticationService.getAuthorizationHeader(req, (err, login) => {
        if (err || login.type !== 'basic') return res.fail('invalid authentication type', Nodee.param.error.http.BAD_REQUEST, Nodee.param.error.detail.INVALID_AUTH_TYPE);
        
        // validate user
        (new User()).findByUsernameAndPassword(login.username, login.password, (err, user) => {
            if (err) return res.fail('an error has occurred while authenticating', Nodee.param.error.http.INTERNAL_SERVER_ERROR);

            if (!user) return res.fail('user not found', Nodee.param.error.http.INVALID_CREDENTIALS, 'USER_NOT_FOUND');
            ...
            (new LoginToken()).saveNewToken(user.userId, req.headers['user-agent'], req.headers['X-Device-Id'], ( (req.body.rememberMe === 1) ? true : false ), (err, token) => {
                if (err || !token) return res.fail('an error has occurred while granting access token', Nodee.param.error.http.INTERNAL_SERVER_ERROR);
